...
let option = (field: t<_, _>, emptyValue) => {
  key: field.key,
  getValue: values => field.getValue(values)->Belt.Option.getWithDefault(emptyValue),
  setValue: (v, values) => field.setValue(Some(v), values),
}
let \"+?|>" = option

let makeListItemField = (default, field, i: int) => {
  key: field.key ++ ("[" ++ (i->string_of_int ++ "]")),
  getValue: values => field.getValue(values)->Belt.List.get(i)->Belt.Option.getWithDefault(default),
  setValue: (v, values) => {
