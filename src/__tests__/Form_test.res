...
let testBe = (expected, value, ()) => {
  open ExpectJs
  expect(value) |> toBe(expected)
}
let testList = (expected, value) =>
  testEqual(expected |> Belt.List.toArray, value |> Belt.List.toArray)
let testStringMap = (expected, value) =>
  testEqual(expected |> Belt.List.toArray, value |> Belt.Map.String.toArray)
let testOptionString = (expected, value, ()) => {
  open ExpectJs
  expect(value |> Belt.Option.getWithDefault(_, "--None--")) |> toEqual(expected)
}
let testMany = (describeMessage, list) =>
  describe(describeMessage, () =>
    Belt.List.forEachWithIndex(list, (index, (message, testItem)) =>
      \"@@"(
        test(message == "" ? describeMessage ++ (" #" ++ string_of_int(index)) : message),
        testItem,
      )
    )
  )
describe("#user form", () => {
  ...
  testMany(
    ...
    list{
      ...
      (
        "lastname get errors",
        testList(list{"Lastname is required."}, getErrors(fields.lastname, form)),
      ),
      ...
      (
        ...
        testList(list{"Firstname is required."}, getErrors(fields.firstname, form)),
      ),
      ...
      (
        "username get errors",
        testList(list{"Username is required."}, getErrors(fields.username, form)),
      ),
      ...
      ("age get errors", testList(list{}, getErrors(fields.age, form))),
      ...
      ("tags get errors", testList(list{}, getErrors(fields.tags->fst, form))),
      ("tags values", testList(list{}, Form.getValues(form).tags)),
      (
        "tags count",
        testEqual(0, (fields.tags->fst).getValue(Form.getValues(form))->Belt.List.length),
      ),
      ...
      (
        "addresses count",
        testEqual(0, (fields.addresses->fst).getValue(Form.getValues(form))->Belt.List.length),
      ),
      ("addresses values", testList(list{}, Form.getValues(form).addresses)),
      (
        "addresses get errors",
        testList(
          list{"Must contains one address at least."},
          getErrors(fields.addresses->fst, form),
        ),
      ),
      ...
      (
        ...
        testEqual(
          2,
          (fields.metadata->fst).getValue(Form.getValues(form))
      ...
      (
        "metadata values",
        testStringMap(
          list{("option1", Metadata.Value.empty), ("option2", Metadata.Value.empty)},
          Form.getValues(form).metadata,
        ),
      ),
      ("metadata get errors", testList(list{}, getErrors(fields.metadata->fst, form))),
      (
        "metadata.option1.title get errors",
        testList(
          list{"Title is required."},
          getErrors((fields.metadata->snd("option1")).title, form),
        ),
      ),
      (
        "metadata.option1.desc get errors",
        testList(list{}, getErrors((fields.metadata->snd("option1")).desc, form)),
      ),
      (
        "metadata.option2.title get errors",
        testList(
          list{"Title is required."},
          getErrors((fields.metadata->snd("option2")).title, form),
        ),
      ),
      (
        "metadata.option2.desc get errors",
        testList(list{}, getErrors((fields.metadata->snd("option2")).desc, form)),
      ),
    },
  )
  ...
  testMany(
    ...
    list{
      ...
      ("lastname get errors", testList(list{}, getErrors(fields.lastname, form))),
      ...
      ("firstname get errors", testList(list{}, getErrors(fields.firstname, form))),
      ...
      ("username get errors", testList(list{}, getErrors(fields.username, form))),
      ...
      (
        "new value of tags",
        testList(list{"tag0", "tag1", "tag2", "tag3", "tag4"}, Form.getValues(form).tags),
      ),
      (
        "tags count",
        testEqual(5, (fields.tags->fst).getValue(Form.getValues(form))->Belt.List.length),
      ),
    },
  )
  ...
  testMany(
    ...
    list{
      (
        ...
        testTrue(
          Form.Eq.metaField(
            Form.getField(fields.lastname.key, form2),
            Form.getField(fields.lastname.key, form3),
          ),
        ),
      ),
    },
